...
public func appleAIPrewarm() -> Bool {
    // Force eager initialization of the on-device model so the first
    // inference is fast.  Without this, SystemLanguageModel.default
    // initializes lazily on the first respond() call.
    let model = SystemLanguageModel.default
    let _ = model.availability
    return true
}
...
private func prepareConversationContext(
    ...
    if lastMessage.role.lowercased() == "user" {
        currentPrompt = lastMessage.content ?? ""
    } else {
        // Last message is assistant (tool_calls) or tool (result): model should continue from transcript.
        // Use the last user message as the prompt so the model has context to generate the final answer.
        currentPrompt = messages.last(where: { $0.role.lowercased() == "user" })?.content ?? "Continue."
    }
