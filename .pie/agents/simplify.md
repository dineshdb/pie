...
logic, dead code, unnecessary abstractions, missed reuse opportunities. Suggest
concrete simplifications with before/after examples. Never suggest changes that
reduce clarity.
...
- Flag inline logic that could use an existing utility (hand-rolled string
...
- **Copy-paste with slight variation**: near-duplicate blocks that should be
  unified
...
- **Unnecessary comments**: comments explaining WHAT the code does (identifiers
  already do that), narrating the change — keep only non-obvious WHY
